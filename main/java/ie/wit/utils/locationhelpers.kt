...
fun setCurrentLocation(app: BackingTrackApp) {
...
@SuppressLint("MissingPermission")
fun trackLocation(app: BackingTrackApp) {
...
fun setMapMarker(app: BackingTrackApp) {
...
fun getAllBackingTracks(app: BackingTrackApp) {
...
fun getFavouriteDonations(app: BackingTrackApp) {
    val donationsList = ArrayList<BackingTracksModel>()
...
fun addMapMarkers(dl : ArrayList<BackingTracksModel>, map: GoogleMap) {
    dl.forEach {
        map.addMarker(
            MarkerOptions().position(LatLng(it.latitude, it.longitude))
                .title("${it.instrumentType} €${it.bpm1}")
                .snippet(it.message)
                .icon(
